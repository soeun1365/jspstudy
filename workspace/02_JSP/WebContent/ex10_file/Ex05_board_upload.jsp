...
<body>
	...
	작성자: <%=dto.getWriter()%>
	<% 
	switch ( dto.getFilename().substring(dto.getFilename().lastIndexOf(".") + 1).toLowerCase() ) {	//확장자를 소문자로 빠꿔라
	case "jpg":	case "jpeg": case "png": case "gif": case "tif": case "bmp":
		out.println("<h3>첨부 이미지</h3>");
		out.println("<img src=\"../" + DIR + "/" + dto.getFilename() + "\">");
		break;
	default:
		out.println("<h3>첨부 파일명: " + dto.getFilename() + "</h3>");	//첨부한것이 이미지가 아닐경우
	}
%>
