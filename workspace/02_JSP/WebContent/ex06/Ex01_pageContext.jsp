<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%--
    	...
    	2. 각 내장 객체는 데이터를 유지하는 생명주기(Life Cycle)가 다르다.
    	...
    	4. 내장객체 종류
			...
			4) application
				(1) 웹 애플리케이션에서 사용
				(2) 웹 애플리케이션을 닫으면 소멸 - Life Cycle이 가장긺
		5. 데이터 저장 메소드
			1) 내장객체.setAttribute('속성명', 데이터);
			2) 저장되는 모든 데이터의 타입이 Object이다.
		6. 데이터 반환 메소드
			1) 내장객체.getAttribute('속성명');
			2) getAttrubute()로 읽어 들인 속성은 오브젝트 타입이라 캐스팅(형 변환)을 해야한다. 
		7. 4개 영역의 데이터는 EL(Express Lnaguage)로 사용한다.
...
<body>
	...
	<%
		// 캐스팅
		String name = (String)pageContext.getAttribute("name");		//object -> String
		int age = (int)pageContext.getAttribute("age");				//object -> int		<비교> parseInt 이거는 String -> int
	%>
