...
<%
	...
	// ProductDTO를 여러개 저장 할 수 있는 cart를 만든다.
	// 1. session에 저장되어있는 cart를 꺼낸다.
	// 2. session에 없으면 개때 새로 cart를 만든다. 
	ArrayList<ProductDTO> cart = (ArrayList<ProductDTO>)session.getAttribute("cart");
	
	if (cart == null) {	//session에 카트가 없다. 즉 첫 장바구니라면
		cart = new ArrayList<>();		//세션에 카트를 만들어준다.
