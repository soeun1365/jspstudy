...
public class MyServlet extends HttpServlet {	//HttpServlet 클래스를 상속 받으면 서블릿이다.
	 ...
	 * 		3) 사용자들이 요청한 파라미터(검색어, 아이디/비번등...), 사용자들의 접속 IP 등... 을 처리
	 * 
	 * 		- 매개변수2
	 * 		1) HttpServletResponse response
	 * 		2) 서버의 응답을 처리하는 객체이다.
	 * 		3) 사용자들의 요청을 처리한 결과(검색결과, 로그인 성공 유무 등...) alert창을 많이 씀
	 * 
	 * 		- service() 메소드가 없으면 doGet() 또는 doPost() 메소드가 호출된다. (기본값은 doGet()이 호출됨)
	 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.getWriter().append("Served at: ").append(request.getContextPath());	//웹브라우저에 보여줄 결과(response)
	}
	 ...
	 * 		- 실제로는 아무처리도 하지 않고, 모든 처리를 doGet()메소드에 위임한다.
	 */
