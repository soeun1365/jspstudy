...
public class HomeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request, response 기본처리
		request.setCharacterEncoding("utf-8");
		response.setContentType("text/html; charset=utf-8");
		
		//요청확인과정
		String requestURI = request.getRequestURI();	/*		/06_MVC/date.do 이런게 옴		*/
		/* response.getWriter().write(requestURI);	확인해보는 코드 */
		String[] arr = requestURI.split("/");	/*	{ "", "06_MVC", "date.do" }	*/
		String command = arr[arr.length - 1];
		/* response.getWriter().write(command);		확인해보는 코드 */
		...
		switch(command) {
		case "date.do" : 
			service = new DateService();		//Model을 선택
