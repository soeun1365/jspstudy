...
public class SelectListCommand3 implements BoardCommand {
	...
	public ModelAndView execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String paging = Paging.getPaging("/11_MYBATIS/selectList3.do", totalRecord, recordPerPage, page);	//페이징클래스
		request.setAttribute("paging", paging);
		
		request.setAttribute("totalRecord", totalRecord);
		
		request.setAttribute("seq", totalRecord - (page - 1) * recordPerPage);
		return new ModelAndView("board/selectList3.jsp", false);	//forward는 contextPath 적지않는다.
	}
